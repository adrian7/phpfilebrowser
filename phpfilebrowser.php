 ...
 */
function env_get_cfg($var, $parseint=FALSE){
 ...
 * @param bool $subdirectories (optional)
 * @return array
 */
...
function get_ignore_files_list(){

    global $ignoreFilesList;

    if( !is_array($ignoreFilesList) ) $ignoreFilesList = array();

    $ignorelist = array_merge($ignoreFilesList, array(SBFILE, 'cache.dirtree') );
    $finallist    = array();

    foreach($ignorelist as $relative) $finallist[] = ( SPATH . '/' . $relative );
...
function display_dir(){ 
        ...
        $ignorelist = get_ignore_files_list();
		...
		if ($handle = opendir($path) ){
 			...
 			while (false !== ($file = readdir($handle)) ){
			   	if ($file != "." && $file != ".."){
                    ...
                    //ignored files
                    if( in_array($abspath, $ignorelist) ) continue;
                    ...
                    $eurl      =  urlencode($fileurl);

                    $anchor  = (SBFILE . '?' . $action .'='. $eurl );
                    $dlurl     = (SBFILE . '?download='. $eurl );
                    $rmurl     = (SBFILE . '?delete='. $eurl );    ?>
			    	
			    	<a class="<?php echo $csscls; ?>" href="<?php echo $anchor; ?>" data-downloadurl="<?php echo $dlurl; ?>"  data-deleteurl="<?php echo $rmurl; ?>" data-path="<?php echo $eurl; ?>">
	...
	if( is_playable_file($path) ): /*play the image/embed/video/audio*/ 
		...
		if ( is_sound($path) ): $type='sound'; ?>
			<audio controls  class="audio">
			 	...
			 	Sorry, your browser does not supports this audio format: <em><?php echo get_file_ext($path); ?></em>.
			 	<a href="<?php echo $fileurl; ?>">Download it here.</a>
			</audio>
		<?php endif;
...
button, input[type=button], input[type=submit], a.button, input[type=file]{
	display: inline-block;
	white-space: nowrap;
...
button:hover, input[type=button]:hover, input[type=submit]:hover, a.button:hover, input[type=file]:hover{
...
button:active, input[type=button]:active, input[type=submit]:active, a.button:active, input[type=file]:active{
...
.pdf .icon, li.file.pdf{
	background-image:url('http://cdn1.iconfinder.com/data/icons/oxygen/128x128/mimetypes/application-pdf.png');
}
...
<body>
	<?php endif; if ( is_array($tree) and count($tree) ): $k = $k*2048;  $ignorelist = get_ignore_files_list(); ?>
	<ul class="<?php echo $class; ?>">
  		<?php foreach ($tree as $node): $liclass = 'file'; $k++;

            if( in_array($node['path'], $ignorelist) ) continue;

	  		if ( is_downloadable_file($node['path']) )
...
<?php endif; if ( $k < 2048 ): /*calles for the first time*/ ?>	
	...
	$(document).ready(function(){
		...
		$('li.file a').click(function(e){
			e.preventDefault(); window.parent.location.href = $(this).attr('rel');
		});

        $('li.dir a').dblclick(function(e){
            e.preventDefault(); $(this).unbind('click'); window.parent.location.href = $(this).attr('rel');
        });
...
if ( isset($_GET['delete']) ) { //delete action
    ...
    if( is_dir($path) )
		recursive_remove_directory(SPATH . "/" . $path);
...
if( isset($_GET['move']) ){ //move action //TODO debug

    $destpath = ( SPATH . rtrim( urldecode($_GET['move']), "/" ) . "/" );
    $itemscnt = intval($_COOKIE['itemstomove']);

    //die("itemstomove = {$itemscnt}");

    if ( $itemscnt > 0 ) for($i=0; $i<$itemscnt; $i++){

        $cname = ('moveitem_' . strval($i));

        $path   = urldecode($_COOKIE[$cname]);
        $opath = (SPATH . $path);

        //die("opath = {$opath} | destpath=" . $destpath . basename($path));

        if( is_file($opath) or is_dir($opath) )
            @rename( $opath, ($destpath . basename($path)) );


        @setcookie($cname, "", time()-3600); //expire the cookie

    }

    @setcookie('itemstomove', 0, time()-3600);

    $redirect = ( SBFILE . '?view=' . $_GET['move'] );

    header("Location: {$redirect}");

}

if ( isset($_GET['upload']) ){ //upload action

	if ( (ADMIN_PASS != 'none') and empty($_SESSION['xfbrowserauth']) ) die("...");

	$upload_dir = urldecode($_GET['upload']);

    if( !is_dir(SPATH . $upload_dir) )
        $upload_dir = substr($upload_dir, 0, strrpos($upload_dir, "/")+1);

    $upload_dir = (SPATH . $upload_dir);
	
	if ( count($_FILES['fileupload']) ){
		...
		if( empty($File['error']) and @move_uploaded_file($File['tmp_name'], $upload_file) ){

           $Success = 1;

        }
        else{
            //--- handle most common file upload errors ---//

            if( $File['error'] == UPLOAD_ERR_INI_SIZE ){
                $sysmaxsize  = fmt_filesize( env_get_cfg('upload_max_filesize', true) );
                $Error          = "The {$File['name']} is too large. Please upload files smaller then {$sysmaxsize}";
            }

            if( $File['error'] == UPLOAD_ERR_CANT_WRITE )
                $Error = "Fatal error: can't write to the temporaty folder, set for file uploads. Please review your server's php.ini settings and try again.";

            if( $File['error'] == UPLOAD_ERR_EXTENSION )
                $Error = "The {$File['name']} cound not be uplooaded due to file extension restrictions.";
...
<title>PHPFileBrowser - Upload</title>
...
a.file .remove{
	background: url('http://cdn1.iconfinder.com/data/icons/freeapplication/png/24x24/Close.png');
	color: #FFF;
	position:relative; 
	top:-50px;
	left:0px;
...
uploadIndex			     = 0;
previewedFilesCount  = 0;

uploadEnded = false;
uploadURL	= '<?php echo SBFILE ?>?upload=<?php echo urlencode($_GET['upload']); ?>';

uploadProgress 		= '#uploadProgress';
previewsContainer	= '#uploadsPreviews';


function filenameBuilder(filename, filedata, mime, boundary){
	var dashdash = '--';
    var crlf = '\r\n';
...
function getFileIndexBySize(size) {
...
function getFileIndex(e, context){

	if (e.target.index == undefined)
		var index = getFileIndexBySize(e.total);
...
function getFileExt(filename){
	if ( filename.indexOf(".") ){
		ext = filename.substr(filename.lastIndexOf(".")+1); return ext.toLowerCase();
	}
...
function previewFile(e) {
	...
	if( (previewsContainer != null) && (typeof previewsContainer != 'undefined') ){ //show the preview icon

		var html = null;
		
		e.target.index = getFileIndex(e, 'preview');
		file 				 = Files[e.target.index];
		icon 				 = getFileExt(file.name);
		
		html = ['<a class="file ', icon, '" id="file_preview_', previewedFilesCount, '", title="', file.name, '">'].join('');
		html+= ('<span class="icon">&nbsp;</span>');
		html+= [ '<span class="remove" onclick="removeFromQueue(', previewedFilesCount, ')" data-id="', previewedFilesCount, '">&nbsp;</span></a>'].join('');
		
		$(previewsContainer).append(html);
...
function removeFromQueue(id){
	...
	Files = newFiles; filesCount --; previewedFilesCount--;
	
	$('#file_preview_' + id).fadeOut('fast', function(){ $('#file_preview_' + id).remove(); });
}

function uploadFile(e){
    
    e.target.index = getFileIndex(e, 'upload');
    ...
    if (typeof newName === "string") {
      builder = filenameBuilder(newName, e.target.result, mime, boundary);
    } else {
      builder = filenameBuilder(file.name, e.target.result, mime, boundary);
    }
    ...
    xhr.onload = function() {
          ...
          if (xhr.responseText) {
          		...
          		$(uploadProgress).val(uploadedFilesCount);
          		
          		if(xhr.responseText != '1')  showError(xhr.responseText);
          }
            
         //Pass any errors to the serverError callback
         if( xhr.status != 200 )
        	 showError("Server error:" + xhr.statusText);
      
         //finished uploading
         if( (uploadIndex == filesCount) && !uploadEnded ){
             showMsg('Upload has finished'); uploadEnded=true; Files = [];
         }
      
    };
	
    uploadIndex++; //increment upload index
	
}

function showError(msg){
	$('<p class="msg warn">' + msg + '</p>').insertAfter('.upload-errors');
}

function showMsg(msg){
	$('<p class="msg info">' + msg + '</p>').insertAfter('.upload-errors');
}

function showPreviews(userFiles){

	$('#dropTarget span.text').fadeOut('fast', function(){
		$(this).remove();
	});
	
	for (var k=0; k<userFiles.length; k++) {
		
		var fileReader = null; //create new filereader for each file
		
		fileReader 			    = new FileReader();
		fileReader.onload  = previewFile;
			
		fileReader.readAsDataURL(userFiles[k]);
			
		Files[filesCount] = userFiles[k]; filesCount++; 
			
		$(uploadProgress).attr('max', filesCount);
	}
...
function startUpload(){
	...
	if(Files.length){
        ...
        $('.drag-and-drop, .choose-files, .upload-files').hide('fast');
        ...
        for (var k=0; k<Files.length; k++) {
            ...
            fileReader.readAsBinaryString(Files[k]);
...
function close_upload_overlay(){
    $('.upload-loading').hide('fast');
    $('.drag-and-drop, .choose-files, .upload-files').show();
    $('#dropTarget span.text').fadeIn('fast');
...
$(document).ready(function(){
	...
	catch(e){ alert("FileReader browser support missing!"); }
	//--- test for filereader support ---//
	...
	} catch (e) { alert("Oups! Your browser does not support xhr.sendAsBinary feature!"); }   
	//--- extend XHR prototype ---//
});
...
<div class="wrap">
	<div id="main">
		<div class="upload-wrap">
			<p><strong>Upload Folder: <?php echo urldecode($_GET['upload']); ?></strong></p><br/>
			
			<div class="upload-loading">
					<h3 align="center">uploading in progress</h3>
                    <p align="center" class="upload-progress-wrap">
                        <progress id="uploadProgress" value="0" max="1" title="upload progress.."></progress>
                    </p>
                ...
                <p align="center">
                    <button class="overlay-close" onclick="close_upload_overlay();">Upload more</button>
                    &nbsp;&nbsp;
                    <button class="window-close" onclick="close_window();">Exit</button>
                </p>

			</div>
			
			<div class="drag-and-drop" id="dropTarget" class="drop-target" ondragenter="return false;" ondragover="return false" ondrop="filesDropped(event);">
				<output id="uploadsPreviews" class="output-uploads-previews" name="uploads_dragged[]"></output> 
				<span class="text"><br/>~ drag and drop files here, or use the button below ~</span>
			</div>
			
			<br class="clear"/>
			
			<p class="choose-files">
				<input type="file" id="inputChooseFiles" size="999" multiple="true" onchange="showPreviews(this.files);"/>
				<button type="button" id="inputAfterChooseFiles">Choose files...</button>
			</p>
			
			<br class="clear"/><br class="clear"/>
			
			<p align="center" class="upload-files">
				<button class="submit" style="font-size: 1.5em;" onclick="startUpload();"><strong>UPLOAD'EM NOW!</strong></button>
			</p>
		</div>
	</div>
</div>
...
.browser a:hover, .browser a.hovered, .browser a.context-menu-active{
...
.browser a:nth-child(5n+1), .browser a:first-child{
    margin-left: 3.5%;
}

/*Context menu*/
.context-menu-item.icon-download{
    background-image: url('http://cdn1.iconfinder.com/data/icons/CornerStone/PNG/arrow%20down.png');
...
.filemenu .paste-file a{
    background-image: url('http://cdn1.iconfinder.com/data/icons/musthave/48/Paste.png');
}
...
function confirm_delete(link){
	if( confirm("Are you sure you want to delete this item?") ) window.location.href = link;
}
...
function show_popup(url){
	...
	try{
		...
		alert("Your browser has blocked the popup.\nNotice the message from the top right corner of the address bar, and allow the window to open.");
	}
}
...
function register_cut_item(itemurl){

    numitems                = cutItems.length;
    cutItems[numitems] = itemurl;

    $.get('<?php echo SBFILE; ?>', {cookie: ('moveitem_' + numitems), v: itemurl, e:1});
    $.get('<?php echo SBFILE; ?>', {cookie: 'itemstomove', v: (numitems+1), e:1});

}

function ui_make_grid(itemsId, minHeight, perRow){

    var minHeight  = parseInt(minHeight);
    var maxHeight = minHeight;
    var perRow	  = parseInt(perRow);
    ...
    if( $(itemsId).length > 0 ){
        ...
        $(itemsId).each(function(){
            ...
            row = i <= perRow ? 1 : parseInt(i/perRow) +1;

            classRow = ('grid-row-' + row);
            classItem= ('grid-item-' + i);

            elemHeight = parseInt( $(this).height() );

            $(this).addClass(classItem);
            $(this).addClass(classRow);

            if (typeof rowsHeights[row-1] == 'undefined')
                rowsHeights[row-1] = maxHeight;
        ...
        if( rowsHeights.length) for (x in rowsHeights){
            classRow = ('grid-row-' + j);  $('.' + classRow).css({height: rowsHeights[x]+'px'}); j++;
        }
    }
...
$(document).ready(function(){
    ...
    $.contextMenu({
        selector: ".browser a",
        autoHide: true,
        show: function(opt){ this.addClass('hovered'); },
        hide : function(opt){ this.removeClass('hovered'); },
        items: {
            download: {name: "Download", icon: "download", callback: function(key, opt){ window.location.href = this.data('downloadurl'); }},
            cut: {name: "Cut", icon: "cut", callback: function(key, opt){ this.addClass('cut'); register_cut_item(this.data('path'))  }},
            delete: {name: "Delete", icon: "delete", callback: function(key, opt){ confirm_delete(this.data('deleteurl')); }}
        }
    });
    //--- attach context menus ---//
	...
	<div id="main">
		...
		<div class="filemenu">
			<ul>
				...
				<?php endif;?>
                <?php if ( ( $_COOKIE['itemstomove'] > 0 ) and $isDir ): ?>
                <li class="paste-file"><a title="paste here" href="<?php echo ( SBFILE . '?move=' . urlencode($_GET['view']) ) ?> ">&nbsp;</a></li>
                <?php endif;?>
                <?php if ( $isDir ): ?>
				<li class="upload-file"><a title="upload file" href="#upload" onclick="show_popup('<?php echo ( SBFILE . '?upload=' . urlencode($_GET['view']) );?>')">&nbsp;</a></li>
			    <?php endif; ?>
             </ul>
		</div>
